...
class AddressBook(UserDict):
    ...
    def func_find(self, name):
        if name in self.contacts:
            print('{:^120}'.format("-"*180))
            print('{:^30}|{:^30}|{:^30}|{:^30}|{:^30}|{:^30}'.format("Name", "Phone", "Email", "Birthday", "Tag", "Notes"))
            print('{:^120}'.format("-"*180))
            print('{:^30}|{:^30}|{:^30}|{:^30}|{:^30}|{:^30}'.format(name, self.check_value(self.contacts[name][0]), self.check_value(self.contacts[name][1]), self.check_value(self.contacts[name][2]), self.check_value(self.contacts[name][3]), self.check_value(self.contacts[name][4])))
        else:
    ...
    def func_search(self, keyword):
        ...
        for key, value in self.contacts.items():
            if keyword.lower() in key.lower() or keyword in value[0] or keyword in value[0].replace(" ", "") or keyword.lower() in value[1].lower() or keyword in value[2] or keyword.lower() in value[3].lower() or keyword.lower() in value[4].lower():
                print('{:^30}|{:^30}|{:^30}|{:^30}|{:^30}|{:^30}'.format(key, self.check_value(self.contacts[key][0]), self.check_value(self.contacts[key][1]), self.check_value(self.contacts[key][2]), self.check_value(self.contacts[key][3]), self.check_value(self.contacts[key][4])))
                contact_counter += 1
    ...
    def func_show_all(self):
        if not self.contacts:
            ...
            print('{:^180}'.format("-"*180))
            print('{:^30}|{:^30}|{:^30}|{:^30}|{:^30}|{:^30}'.format("Name", "Phone", "Email", "Birthday", "Tag", "Notes"))
            print('{:^180}'.format("-"*180))
            for name, contact in self.contacts.items():
                print('{:^30}|{:^30}|{:^30}|{:^30}|{:^30}|{:^30}'.format(name, self.check_value(contact[0]), self.check_value(contact[1]), self.check_value(contact[2]), self.check_value(contact[3]), self.check_value(contact[4])))
    ...
    @input_error
    def func_add(self, name, phone=None, email = None, birthday = None, tag = None, notes = None):
        if len(name) == 0:
            raise KeyError
        else:
            new_contact = Record(Name(name), Phone(phone), Email(email), Birthday(birthday), Tag(tag), Notes(notes))
            self.contacts[new_contact.name.value] = [new_contact.phone.value, new_contact.email.value, new_contact.birthday.value, new_contact.tag.value, new_contact.notes.value]
            print('{:^180}'.format("-"*180))
            print('{:^30}|{:^30}|{:^30}|{:^30}|{:^30}|{:^30}'.format("Name", "Phone", "Email", "Birthday", "Tag", "Notes"))
            print('{:^180}'.format("-"*180))  
            print('{:^30}|{:^30}|{:^30}|{:^30}|{:^30}|{:^30}'.format(name, self.check_value(self.contacts[name][0]), self.check_value(self.contacts[name][1]), self.check_value(self.contacts[name][2]), self.check_value(self.contacts[name][3]), self.check_value(self.contacts[name][4])))
    ...
    def func_birthday(self, name):
        if name in self.contacts:
            contact = Record(name, self.contacts[name][0], self.contacts[name][1], self.contacts[name][2], self.contacts[name][3], self.contacts[name][4])
    ...
    while True:
        ...
        if listen in OPERATIONS_MAP:
            if listen == "add":
                ...
                OPERATIONS_MAP[listen](name, phone, email, birthday, tag, notes)
